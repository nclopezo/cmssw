...
int DTPosNeg::getPN( int whe, int sec, int sta ) {
  int p_n;
  int c_t;
...
int DTPosNeg::getCT( int whe, int sec, int sta ) {
  int p_n;
  int c_t;
  decode( getData( whe, sec, sta ), p_n, c_t );
  return c_t;
...
int DTPosNeg::idCode( int whe, int sec, int sta ) {
  return ( ( ( ( whe + 3 ) * 100 ) + sec ) * 10 ) + sta;
}


int DTPosNeg::pnCode( int p, int t ) {
  return ( p * 1000 ) + t;
}


void DTPosNeg::decode( int code, int& whe, int& sec, int& sta ) {
  whe = ( code / 1000 ) - 3;
  sec = ( code /   10 ) % 100;
  sta =   code          % 10;
  return;
...
void DTPosNeg::decode( int code, int& p, int& t ) {
  p = code / 1000;
  t = code % 1000;
  return;
}
