...
void SiPixelGainCalibrationRejectNoisyAndDead::fillDatabase(const edm::EventSetup& iSetup){
  ...
  if(DEBUG) cout<<"=>=>=>=> Starting the function fillDatabase()"<<endl;
  ...
  if(gainhi_>21) gainhi_=21;
  if(pedlow_<-100) pedlow_=-100;
  if(pedhi_>300) pedhi_=300;
  
  std::cout<<"New payload will have pedlow,hi "<<pedlow_<<","<<pedhi_<<" and gainlow,hi "<<gainlow_<<","<<gainhi_<<endl;
  if(record_=="SiPixelGainCalibrationOfflineRcd")
  ...
  for(std::map <int,std::vector<std::pair<int,int> > >::const_iterator it=noisypixelkeeper.begin();it!=noisypixelkeeper.end();it++){
    ...
    if(!willNoisyPixBeInserted)
      cout<<"All Noisy Pixels in detid "<<it->first<<"won't be inserted, check the TrackerGeometry you are using !! You are missing some modules"<<endl;
  }
...
// 	bool isColumnDeadNew = false;
	...
	bool isPixelNoisy = false;
        ...
        if(DEBUG) cout<<"=>=>=>=> Trying to get gain/ped "<<endl;
	  ...
	  cout<<"Problem trying to catch gain/ped from DETID "<<detid<< " @ col,row "<<icol<<","<<jrow<<endl;
	}
	//std::cout<<"For DetId "<<detid<<" we found gain : "<<gain<<", pedestal : "<<ped<<std::endl;
	
        if(DEBUG) cout<<"=>=>=>=> Found gain "<<gainforthiscol[iglobalrow]<<" and ped "<<pedforthiscol[iglobalrow]<<endl;
	...
	if(isPixelNoisy) cout<<"Inserting a noisy pixel in "<<detid<<" at col,row "<<icol<<","<<jrow<<endl;
	if(isColumnNoisy) cout<<"Inserting a noisy column in "<<detid<<" at col,row "<<icol<<","<<jrow<<endl;
	if(isPixelNoisy) nnoisy++;
	if(isPixelDead) ndead++;
	
	if(DEBUG) cout<<"=>=>=>=> Now Starting to fill the DB"<<endl;


	
	//**********  Fill the new DB !!
	
	if(DEBUG) cout<<"=>=>=>=> Filling Pixel Level Calibration"<<endl;
	...
	//Set Gain
	  ...
	  if(DEBUG) cout<<"=>=>=>=> Filling Column Level Calibration"<<endl;
	  ...
	  if(isColumnDead){
	    ...
	    if(record_=="SiPixelGainCalibrationForHLTRcd")
	      theGainCalibrationDbInputForHLT_->setData(pedforthiscol[iglobalrow],gainforthiscol[iglobalrow],theSiPixelGainCalibrationPerCol);
	  }
	}
	
	if(DEBUG) cout<<"=>=>=>=> This pixel is finished inserting"<<endl;

      }//end of loop over rows
      
      if(DEBUG) cout<<"=>=>=>=> This column is finished inserting"<<endl;
    ...
    if(record_=="SiPixelGainCalibrationForHLTRcd"){
      SiPixelGainCalibrationOffline::Range hltrange(theSiPixelGainCalibrationPerCol.begin(),theSiPixelGainCalibrationPerCol.end());
      if( !theGainCalibrationDbInputForHLT_->put(detid,hltrange,ncols) )
        edm::LogError("SiPixelGainCalibrationAnalysis")<<"warning: detid already exists for HLT (gain per col, ped per col) calibration database"<<std::endl;
    }
    
    if(DEBUG) cout<<"=>=>=>=> This detid is finished inserting"<<endl;
  ...
  if(!mydbservice.isAvailable() ){
    ...
    if(record_=="SiPixelGainCalibrationForHLTRcd"){
      ...
      if( mydbservice->isNewTagRequest("SiPixelGainCalibrationForHLTRcd") ){
        ...
        mydbservice->appendSinceTime<SiPixelGainCalibrationForHLT>(  theGainCalibrationDbInputForHLT_, 
								     mydbservice->currentTime(),
								     "SiPixelGainCalibrationForHLTRcd");
      }
    }
